jQuery(document).ready(function($) {
    ...
    function refreshCaptcha() {
        var captchaElement = $('.cpp-captcha-code'); var captchaInput = $('#captcha_input'); if (!captchaElement.length) return;
        ...
        $.post(cpp_front_vars.ajax_url, { action: 'cpp_get_captcha', nonce: cpp_front_vars.nonce }, function(response) {
            if (response.success && response.data && response.data.code) captchaElement.text(response.data.code); else captchaElement.text('خطا');
        }).fail(function() { captchaElement.text('خطا'); });
    }

    $(document).on('click', '.cpp-order-btn', function() {
        var button = $(this); var modal = $('#cpp-order-modal');
        modal.find('#cpp-order-product-id').val(button.data('product-id')); modal.find('.cpp-modal-product-name').text(button.data('product-name')); modal.find('.cpp-modal-product-location').text(button.data('product-location') || ''); modal.find('.cpp-modal-product-unit').text(button.data('product-unit') || ''); 
    ...
    $('#cpp-order-form').on('submit', function(e) {
        e.preventDefault(); var form = $(this); var btn = form.find('button[type="submit"]'); var formData = form.serialize(); 
        btn.prop('disabled', true).text(cpp_front_vars.i18n.sending || 'در حال ارسال...'); form.find('.cpp-form-message').remove();
        $.post(cpp_front_vars.ajax_url, formData + '&action=cpp_submit_order&nonce=' + cpp_front_vars.nonce, function(response) {
            if (response.success) { form.before('<div class="cpp-form-message cpp-success">' + response.data.message + '</div>'); setTimeout(function() { $('#cpp-order-modal').hide(); btn.prop('disabled', false).text('ثبت درخواست'); }, 2000); } 
            else { form.before('<div class="cpp-form-message cpp-error">' + (response.data.message || 'خطا') + '</div>'); btn.prop('disabled', false).text('ثبت درخواست'); if (response.data.code === 'captcha_error') refreshCaptcha(); }
        }).fail(function() { form.before('<div class="cpp-form-message cpp-error">' + cpp_front_vars.i18n.server_error + '</div>'); btn.prop('disabled', false).text('ثبت درخواست'); });
    });
     ...
     $(document).on('click', '.cpp-chart-btn', function(e) {
        ...
        if (cpp_front_vars.logo_url) {
            modal.find('.cpp-chart-bg').css({
                'background-image': 'url(' + cpp_front_vars.logo_url + ')', 'background-repeat': 'no-repeat', 'background-position': 'center center', 'background-size': '200px', 'opacity': '0.1',
        ...
        }).fail(function() { chartContainer.find('.chart-loading').remove(); chartCanvas.hide(); chartContainer.prepend('<p class="chart-error">خطای سرور</p>'); });
    });

    $(document).on('click', '.cpp-chart-filter', function() {
        $('.cpp-chart-filter').removeClass('active'); $(this).addClass('active');
    ...
    $(document).on('click', '.cpp-view-more-btn', function() {
        var button = $(this); var wrapper = button.closest('.cpp-grid-view-wrapper'); var nextPage = parseInt(button.data('page'), 10) + 1;
        button.prop('disabled', true).text(cpp_front_vars.i18n.loading);
        $.post(cpp_front_vars.ajax_url, { action: 'cpp_load_more_products', nonce: cpp_front_vars.nonce, page: nextPage, shortcode_type: button.data('shortcode-type') }, function(response) {
            if (response.success && response.data.html) { wrapper.find('.cpp-grid-view-table tbody').append(response.data.html); button.data('page', nextPage).prop('disabled', false).text(cpp_front_vars.i18n.view_more); if (!response.data.has_more) { button.text(cpp_front_vars.i18n.no_more_products).prop('disabled', true).parent().hide(); } } 
            else { button.text(cpp_front_vars.i18n.no_more_products).prop('disabled', true).parent().hide(); }
        }).fail(function() { alert(cpp_front_vars.i18n.server_error); button.prop('disabled', false).text(cpp_front_vars.i18n.view_more); });
    });
